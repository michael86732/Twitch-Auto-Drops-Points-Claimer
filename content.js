...
// which can have different text based on language (e.g., 'Claim Now', '立即領取', '现在领取', 'Jetzt abholen', 'Reclamar ahora', 'En profiter', 'Riscuoti subito', '今すぐ入手', ' sickening 받기', 'Nhận ngay', 'Şimdi Al', 'Vyzvednout kořist', 'Διεκδίκηση τώρα', 'Kiváltás most!', 'Получить сейчас', 'เคลมตอนนี้', 'Claim nu', 'Hent nå', 'Odbierz teraz', 'Resgatar agora', 'Vyzdvihnúť teraz', 'Solicită acum', 'Lunasta nyt', 'Hämta nu')
// It also claims Channel Points bonus buttons, supporting different aria-label values.

// Use an IIFE to create a private scope and prevent variable conflicts
// Check if the script has already been initialized in this frame
if (window.twitchAutoClaimerInitialized) {
    console.log("Twitch Auto Drops Claimer content script already initialized in this frame. Exiting.");
} else {
    window.twitchAutoClaimerInitialized = true; // Set flag to indicate initialization

    console.log("Twitch Auto Drops Claimer content script loaded and initializing.");
    ...
    function claimRewards() {
      ...
      if (currentTime - lastDropClaimTime < dropClaimCooldown) {
          ...
          const dropClaimButton = document.evaluate(
            "//button[.//div[@data-a-target='tw-core-button-label-text' and (text()='Claim Now' or text()='立即領取' or text()='现在领取' or text()='Jetzt abholen' or text()='Reclamar ahora' or text()='En profiter' or text()='Riscuoti subito' or text()='今すぐ入手' or text()='지금 받기' or text()='Nhận ngay' or text()='Şimdi Al' or text()='Vyzvednout kořish' or text()='Διεκδίκηση τώρα' or text()='Kiváltás most!' or text()='Получить сейчас' or text()='เคลมตอนนี้' or text()='Claim nu' or text()='Hent nå' or text()='Odbierz teraz' or text()='Resgatar agora' or text()='Vyzdvihnúť teraz' or text()='Solicită acum' or text()='Lunasta nyt' or text()='Hämta nu')]]",
            document,
            null,
      ...
      if (currentTime - lastPointsClaimTime < pointsClaimCooldown) {
          ...
          // Updated XPath selector for Channel Points "Claim Bonus" button to support multiple aria-label values.
          // This selector relies on the aria-label which is often language-independent, but can vary.
          const pointsClaimButton = document.evaluate(
              "//button[@aria-label='Claim Bonus' or @aria-label='領取額外獎勵' or @aria-label='Få bonus' or @aria-label='Bonus einfordern' or @aria-label='Reclamar bonificación' or @aria-label='Riscatta bonus' or @aria-label='Bónusz igénylése' or @aria-label='Bonus claimen' or @aria-label='Motta bonus' or @aria-label='Odbierz bonus' or @aria-label='Receber bónus' or @aria-label='Obține bonusul' or @aria-label='Vyzdvihnúť bonus' or @aria-label='Lunasta bonus' or @aria-label='Hämta bonus' or @aria-label='เคลมโบนัส' or @aria-label='领取奖励' or @aria-label='ボーナスを受け取る' or @aria-label='보너스 받기' or @aria-label='Διεκδίκηση μπόνους' or @aria-label='Получаване на бонус' or @aria-label='Получить бонус']", // Added Thai, Simplified Chinese, Japanese, Korean, Greek, Bulgarian, Russian
              document,
              null,
              XPathResult.FIRST_ORDERED_NODE_TYPE,
              null
          ...
          if (pointsClaimButton) {
              ...
              // if (pointsClaimButton.offsetParent !== null) { // Check if element is visible
                  console.log("Found Channel Points 'Claim Bonus' button (multi-language). Element details:", { // More detailed logging
